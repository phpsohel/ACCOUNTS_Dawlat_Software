...
class RoleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $role->syncPermissions('challan-index');
    ...
    public function setPermission(Request $request)
    {
        ...
        if($request->has('challan-index')){
            $permission = Permission::firstOrCreate(['name' => 'challan-index']);
            if(!$role->hasPermissionTo('challan-index')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('challan-index');

        if($request->has('challan-add')){
                $permission = Permission::firstOrCreate(['name' => 'challan-add']);
                if(!$role->hasPermissionTo('challan-add')){
                    $role->givePermissionTo($permission);
                }
            }
            else
                $role->revokePermissionTo('challan-add');

            if($request->has('challan-edit')){
                $permission = Permission::firstOrCreate(['name' => 'challan-edit']);
                if(!$role->hasPermissionTo('challan-edit')){
                    $role->givePermissionTo($permission);
                }
            }
            else
                $role->revokePermissionTo('challan-edit');

        if($request->has('challan-deleted')){
            $permission = Permission::firstOrCreate(['name' => 'challan-deleted']);
            if(!$role->hasPermissionTo('challan-deleted')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('challan-deleted');
